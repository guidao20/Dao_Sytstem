...
from write_database_pane import write_database_pane
from Eword_search_pane import Eword_search_pane
from Eword_into_pane import Eword_into_pane
from escape_data_pane import escape_data_pane
from knowledge_search_pane import knowledge_search_pane
from knowledge_into_pane import knowledge_into_pane
...
if __name__ == '__main__':
    ...
    write_database_pane = write_database_pane(talk_solve_pane)
    write_database_pane.move(0, talk_solve_pane.height())
    write_database_pane.show()

    mind_pane = mind_pane(main_pane)
    mind_pane.move(0,main_pane.height())
    mind_pane.show()

    escape_data_pane = escape_data_pane(mind_pane)
    escape_data_pane.move(0,escape_data_pane.height())
    escape_data_pane.show()
    ...
    Eword_search_pane = Eword_search_pane(paper_pane)
    Eword_search_pane.move(0,Eword_search_pane.height())
    Eword_search_pane.show()

    Eword_into_pane = Eword_into_pane(Eword_search_pane)
    Eword_into_pane.move(0,Eword_into_pane.height())
    Eword_into_pane.show()

    knowledge_search_pane = knowledge_search_pane(paper_pane)
    knowledge_search_pane.move(0, knowledge_search_pane.height())
    knowledge_search_pane.show()

    knowledge_into_pane = knowledge_into_pane(knowledge_search_pane)
    knowledge_into_pane.move(0, knowledge_into_pane.height())
    knowledge_into_pane.show()
    ...
    def show_escape_pane():
        animation = QPropertyAnimation(escape_data_pane)
        animation.setTargetObject(escape_data_pane)
        animation.setPropertyName(b"pos")
        animation.setStartValue(escape_data_pane.pos())
        animation.setEndValue(QPoint(0, 0))
        animation.setDuration(1000)
        animation.setEasingCurve(QEasingCurve.OutBounce)
        animation.start(QAbstractAnimation.DeleteWhenStopped)

    def exit_escape_pane():
        animation = QPropertyAnimation(escape_data_pane)
        animation.setTargetObject(escape_data_pane)
        animation.setPropertyName(b"pos")
    ...
    def show_Eword_search_pane():
        animation = QPropertyAnimation(Eword_search_pane)
        animation.setTargetObject(Eword_search_pane)
        animation.setPropertyName(b"pos")
        animation.setStartValue(Eword_search_pane.pos())
        animation.setEndValue(QPoint(0, 0))
        animation.setDuration(1000)
        animation.setEasingCurve(QEasingCurve.OutBounce)
        animation.start(QAbstractAnimation.DeleteWhenStopped)

    def exit_Eword_search_pane():
        animation = QPropertyAnimation(Eword_search_pane)
        animation.setTargetObject(Eword_search_pane)
        animation.setPropertyName(b"pos")
        animation.setStartValue(QPoint(0, 0))
    ...
    def show_know_search_pane():
        animation = QPropertyAnimation(knowledge_search_pane)
        animation.setTargetObject(knowledge_search_pane)
        animation.setPropertyName(b"pos")
        animation.setStartValue(knowledge_search_pane.pos())
        animation.setEndValue(QPoint(0, 0))
        animation.setDuration(1000)
        animation.setEasingCurve(QEasingCurve.OutBounce)
        animation.start(QAbstractAnimation.DeleteWhenStopped)

    def exit_know_search_pane():
        animation = QPropertyAnimation(knowledge_search_pane)
        animation.setTargetObject(knowledge_search_pane)
        animation.setPropertyName(b"pos")
        animation.setStartValue(QPoint(0, 0))
    ...
    def show_knowledge_into_pane():
        animation = QPropertyAnimation(knowledge_into_pane)
        animation.setTargetObject(knowledge_into_pane)
        animation.setPropertyName(b"pos")
        animation.setStartValue(knowledge_into_pane.pos())
        animation.setEndValue(QPoint(0, 0))
        animation.setDuration(1000)
        animation.setEasingCurve(QEasingCurve.OutBounce)
        animation.start(QAbstractAnimation.DeleteWhenStopped)

    def show_Eword_into_pane():
        animation = QPropertyAnimation(Eword_into_pane)
        animation.setTargetObject(Eword_into_pane)
        animation.setPropertyName(b"pos")
        animation.setStartValue(Eword_into_pane.pos())
        animation.setEndValue(QPoint(0, 0))
        animation.setDuration(1000)
        animation.setEasingCurve(QEasingCurve.OutBounce)
        animation.start(QAbstractAnimation.DeleteWhenStopped)

    def exit_knowledge_into_pane():
        animation = QPropertyAnimation(knowledge_into_pane)
        animation.setTargetObject(knowledge_into_pane)
        animation.setPropertyName(b"pos")
        animation.setStartValue(QPoint(0, 0))
        animation.setEndValue(QPoint(knowledge_search_pane.width(), 0))
        animation.setDuration(1000)
        animation.setEasingCurve(QEasingCurve.OutBounce)
        animation.start(QAbstractAnimation.DeleteWhenStopped)

    def exit_Eword_into_pane():
        animation = QPropertyAnimation(Eword_into_pane)
        animation.setTargetObject(Eword_into_pane)
        animation.setPropertyName(b"pos")
        animation.setStartValue(QPoint(0, 0))
        animation.setEndValue(QPoint(Eword_search_pane.width(), 0))
        animation.setDuration(1000)
        animation.setEasingCurve(QEasingCurve.OutBounce)
        animation.start(QAbstractAnimation.DeleteWhenStopped)
    ...
    def show_writedatabase_pane():
        animation = QPropertyAnimation(write_database_pane)
        animation.setTargetObject(write_database_pane)
        animation.setPropertyName(b"pos")
        animation.setStartValue(write_database_pane.pos())
        animation.setEndValue(QPoint(0, 0))
        animation.setDuration(1000)
        animation.setEasingCurve(QEasingCurve.OutBounce)
        animation.start(QAbstractAnimation.DeleteWhenStopped)
    ...
    def exit_writedatabase_pane():
        animation = QPropertyAnimation(write_database_pane)
        animation.setTargetObject(write_database_pane)
        animation.setPropertyName(b"pos")
        animation.setStartValue(QPoint(0, 0))
        animation.setEndValue(QPoint(talk_solve_pane.width(), 0))
        animation.setDuration(1000)
        animation.setEasingCurve(QEasingCurve.OutBounce)
        animation.start(QAbstractAnimation.DeleteWhenStopped)
    ...
    write_database_pane.exit_database_signal.connect(exit_writedatabase_pane)
    talk_solve_pane.enter_writedata_signal.connect(show_writedatabase_pane)
    paper_pane.show_english_signal.connect(show_Eword_search_pane)
    Eword_search_pane.exit_search_signal.connect(exit_Eword_search_pane)
    Eword_search_pane.enter_into_signal.connect(show_Eword_into_pane)
    Eword_into_pane.exit_wordbase_signal.connect(exit_Eword_into_pane)
    mind_pane.enter_escape_signal.connect(show_escape_pane)
    escape_data_pane.exit_elution_signal.connect(exit_escape_pane)
    paper_pane.show_knowledge_signal.connect(show_know_search_pane)
    knowledge_search_pane.exit_search_signal.connect(exit_know_search_pane)
    knowledge_into_pane.exit_knowbase_signal.connect(exit_knowledge_into_pane)
    knowledge_search_pane.enter_into_signal.connect(show_knowledge_into_pane)
